...
def load_model(model_name, cache_dir):
    ...
    if 'llama' in model_name:
        ...
        model = AutoModelForCausalLM.from_pretrained(model_name, cache_dir=cache_dir, quantization_config=double_quant_config)
...
def loadFileWithoutMetadata(fn, story_number_limit=-1):
    ...
    for num in range(40):
      story = stories[num]
      story["context"] = re.split('[.?!]', story["context"])
      for sent in story["context"]:
        if sent == '':
            continue
        sent = sent.strip()
        if not sent.endswith("."):
          sent += "."
        d["story"].append(sent)
      for entry in story["questions"]:
        d["question"] = entry["question"]
        d["answer"] = entry["choices"][entry["answer"]]
